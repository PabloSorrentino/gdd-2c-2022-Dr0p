...
-- envio - localidad
CREATE TABLE [Dr0p].[Envios_Ventas_Localidad](
                                                 envio_venta_id DECIMAL(18,0) FOREIGN KEY REFERENCES [Dr0p].[Envios_Ventas](id),
                                                 localidad_id DECIMAL(18,0) FOREIGN KEY REFERENCES [Dr0p].[Localidades](id),
                                                 importe DECIMAL(18,2),
                                                 tiempo_estimado DECIMAL(18,0),
                                                 PRIMARY KEY( envio_venta_id, localidad_id)
)
...
SELECT DISTINCT VENTA_DESCUENTO_CONCEPTO, 0 as IMPORTE
FROM [gd_esquema].[Maestra]
...
--envio - localidad
INSERT INTO [Dr0p].[Envios_ventas_Localidad] (
    envio_venta_id,
    localidad_id,
    importe,
    tiempo_estimado
...
FROM [gd_esquema].[Maestra] M
         ...
         INNER JOIN Dr0p.Envios_Ventas EV ON EV.venta_codigo= M.VENTA_CODIGO
